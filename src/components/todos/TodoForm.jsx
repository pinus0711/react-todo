...
const TodoForm = ({ onAddOrUpdate, onClose, children, todo }) => {
    ...
    const addOrUpdateTodoHandler = () => {
        if (isNewTodoForm(children)) {
            ...
            onAddOrUpdate({ id: self.crypto.randomUUID, title, summary, category });
        } else {
            const updateTodo = {
                id: todo.id,
                title,
    ...
    return (
        <>
            <h3 className="text-3xl text-red-200">{isNewTodoForm(children) ? 'Add' : 'Update'} Todo</h3>
            <form className='my-2'>
                <div>
                ...
                <div className='flex justify-end gap-4'>
                    ...
                    <button className='px-6 py-3 text-xl text-red-200' type='button' onClick={addOrUpdateTodoHandler} disabled={isFormInvalid}>{isNewTodoForm(children) ? 'Add' : 'Update'}</button>
                </div>
            </form>
        </>
    )
